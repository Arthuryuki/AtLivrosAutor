    ...
    public class AutorController : Controller
    {
        ...
        public async Task<IActionResult> Add(AutorViewModel autorViewModel)
        {
            ...
            try
            {
                ...
                using var response = await httpClient.PostAsync("https://localhost:7166/Autor", content);
                response.EnsureSuccessStatusCode();
        ...
        public async Task<IActionResult> Detalhes(int id)
        {
            ...
            {
                httpClient.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", jwtToken);
                using var response = await httpClient.GetAsync($"https://localhost:7166/Autor/{id}");
                response.EnsureSuccessStatusCode();
                var apiResponse = await response.Content.ReadAsStringAsync();
        ...
        public async Task<IActionResult> Editar(int id, AutorViewModel autorViewModel)
        {
            ...
            try
            {
                httpClient.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", jwtToken);
                StringContent content = new StringContent(JsonConvert.SerializeObject(autorViewModel), Encoding.UTF8, "application/json");
                using var response = await httpClient.PutAsync($"https://localhost:7166/Autor/{id}", content);
                response.EnsureSuccessStatusCode();
        ...
        public async Task<IActionResult> Deletar(int id)
        {
            ...
            try
            {
                ...
                using var response = await httpClient.DeleteAsync($"https://localhost:7166/Autor/{id}");
                response.EnsureSuccessStatusCode();
